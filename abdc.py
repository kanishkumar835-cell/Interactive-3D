...
class ObjectDetectorApp(ctk.CTk):
    def __init__(self):
        ...
        self.status_label.configure(text="Status: Loading YOLOv8 model, please wait...")
    ...
    def setup_ui(self):
        ...
        # --- Detect Button ---
        self.detect_button = ctk.CTkButton(self, text="Detect Objects", command=self.run_detection_thread, state="disabled")
    ...
    def browse_file(self):
        ...
        if path:
            ...
            self.last_results = None # Clear previous results

    def run_detection_thread(self):
        """Runs the detection in a new thread."""
    ...
    def detect_objects(self):
        """Performs inference and saves the result."""
        try:
            results = self.model(self.pil_image)
            self.last_results = results[0] # Save the raw results
            self.update_display_with_threshold(self.slider.get())
            self.status_label.configure(text=f"Status: Detection complete. Found {len(self.last_results.boxes)} objects total.")
            
            # Save and open the final image
            final_image_to_save = self.draw_predictions(self.pil_image.copy(), self.last_results, self.slider.get())
            final_image_to_save.save("detection_result.png")
            os.startfile("detection_result.png")
            
        except Exception as e:
            self.status_label.configure(text=f"Error: Detection failed. {e}")
        finally:
            self.detect_button.configure(state="normal")
            self.browse_button.configure(state="normal")

    def update_display_with_threshold(self, threshold_value):
        """Redraws the image and updates text based on the slider's value."""
    ...
    def update_results_text(self, result, threshold):
        """Updates the textbox with a summary of detections."""
        self.results_textbox.configure(state="normal")
        self.results_textbox.delete("0.0", "end")
        text_summary = f"Detections (score > {threshold:.2f}):\n" + "="*30 + "\n"
        
        # Sort results by confidence score for readability
    ...
    def display_image(self, pil_image):
        """Resizes and displays a PIL image in the UI."""
